...
class NotenSkala(var maxPoints: Double = 20.0, var suffPoints: Double = 12.0, var pointPath: Double = 0.5, var gradeRound: Double = 0.5, var myPoints: Double = 16.0) {
    ...
    // main function
    public fun calcGrade(): Double {
        Log.d(LOGTAG, "entering public fun calcGrade($maxPoints, $suffPoints, $myPoints, $pointPath, $gradeRound)")
        //val pointDecs : Byte =
        ...
        Log.d(LOGTAG, "leaving public fun calcGrade($grade)")
        return grade
    }

    // get number of decimals of given number
    private fun getNbrOfDecimals(number : Double) : Byte {
    ...
    private fun calcLinearFunction(p1x : Double, p1y : Double, p2x : Double, p2y : Double) : Map<String,Double>  {
